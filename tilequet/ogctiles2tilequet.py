"""Convert OGC API - Tiles endpoints to TileQuet format.

Fetches pre-rendered tiles from OGC API - Tiles RESTful endpoints.
This is the modern successor to WMTS, using clean REST URLs:

    {baseUrl}/collections/{collectionId}/tiles/{tileMatrixSetId}/{z}/{y}/{x}
...
def _get_http_client():
    ...
    try:
        ...
        raise ImportError(
            "The 'httpx' package is required for OGC API - Tiles conversion. "
...
def convert(
    ...
    *,
    ...
    Fetches tiles from the RESTful URL pattern:
    {base_url}/collections/{collection}/tiles/{tile_matrix_set}/{z}/{y}/{x}

    Args:
        base_url: OGC API base URL (e.g., https://maps.example.com).
        output_path: Path to output .parquet file.
        collection: Collection identifier.
        tile_matrix_set: Tile matrix set ID (default: WebMercatorQuad).
    ...
    # Try to fetch collection metadata for bounds/description
    ...
    if verbose:
        try:
            ...
            if response.status_code == 200:
                ...
                spatial = col_meta.get("extent", {}).get("spatial", {})
                if spatial.get("bbox") and bbox == (-180, -85.051129, 180, 85.051129):
                    b = spatial["bbox"][0]
                    logger.info("Collection bounds: %s", b)
    ...
    try:
        for z in range(min_zoom, max_zoom + 1):
            ...
            for z, x, y in tile_coords:
                tile_url = f"{base_url}/collections/{collection}/tiles/{tile_matrix_set}/{z}/{y}/{x}"

                try:
                    data = _fetch_tile(client, tile_url)
    ...
    if writer.tile_count == 0:
        raise ValueError("No tiles were fetched from the OGC API - Tiles endpoint")
